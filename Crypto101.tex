...
By far the most common native \gls{stream cipher} in common use on desktop
...
%**HMM** It took me a while to find out that key scheduling happens /before/ keystream generation; and that the "pseudorandom generator" does produce a keystream - both facts are never mentioned. I hope it's ok to rename the "pseudorandom generator" to "keystream generator" - the former says something about the form of its result, but the latter stresses its /purpose/. And now, instead of the truism "The pseudorandom generator which produces pseudorandom bytes ..." we get the more informative sentence below. If the renaming is not ok, I would argue that the change should be to "The pseudorandom generator which produces keystream bytes ...", thereby again coupling the result's property (being pseudorandom) with its purpose (being the keystream).
\begin{itemize}
\item Step 1: The key scheduling algorithm, which produces an initial state array
$S$ for a given key.
\item Step 2: The keystream generator, which produces pseudorandom bytes from
...
%**HMM** "mixed in with"? Either "mixed into" (a little strange English, maybe, but conveys the fact that the state is changed), or "mixed with".
Then, the key is mixed into the state. This is done by iterating
...
def key_schedule(key):
    ...
    return s
\end{minted}
\subsection{The keystream generator}
\label{sec-2-3-11-2}

The keystream generator is responsible for producing pseudorandom
bytes from the state $S$ that are used as the keystream for 
encrypting the plaintext. For each index $i$, it computes $j = j +
...
%**HMM** It is unclear to the reader whether the next paragraph still talks about the idea "that knowing even small parts ...", or whether this idea is concluded. Here, this is not so: Therefore, I first wanted to argue that one idea should be confined to one paragraph, and hence, there shouldn't be a paragraph break here. However, this would create a very long paragraph (also the "WEP, ..." paragraph would have to be moved into this one). I think the better solution is to make the reader aware that the idea is coninued, e.g. with this simple addition:
In contrast to RC4, most modern stream ciphers provide a way to combine a long-term key
%**HMM** "nonce" gets an explanation in section 6.12. ("n/umber used /once") that should be moved here, as this is the first mention of this term.
...
approach to add this feature was also the simplest: concatenate the long-term key $k$ and
the nonce $n$: $k \| n$ to create the input key for RC4, taking advantage of RC4's flexible key length
requirements. However, this scheme meant attackers could guess easily some bits of the complete key,
%**HMM** Namely??? the leading zeros of the nonce if the nonce wasn't truncated to the first 1 bit? Or by assuming that a nonce was typically a small number that "counts" the messages sent? Or something else? And we have to distinguish clearly between the "long-term key" and the "complete key". You do this by using "long-term key" and "key" ... I'd, in this short paragraph, use "complete key" to be definitely clear which key ist meant.
allowing them to slowly recover the long-term key from a large amount
of messages (around $2^{24}$ to $2^{26}$, i.e., some tens of millions).
...
%**HMM** And now? Again, I sleep bad. Wouldn't the following be an acceptable remedy for this:
The only solution to RC4's weaknesses is to not use it. Nicely enough,
new and better native stream ciphers have been developed. We will see
