...
git commit with hash \texttt{89e90df}, also known as \texttt{v0.1.1-36-g89e90df}.
...
\item A \emph{padding oracle}: a function that takes arbitrary ciphertexts and tells
the attacker if the padding was correct (we will see later that such oracles are quite
common!)
\end{enumerate}
...
plaintext probably won't have valid padding: the odds are in the
%**HMM** This is not a single block, but two R and C_i. Removing "block" solves the problem.
...
valid padding. They can do that by indirectly playing with the last byte of the
%**HMM** Explaining is hard ;-). Let me, once try to explain what happens when an explanation can lead a reader (me, for example) only slightly astray - I think your explanations are VERY good; but I'd still like to make them a trifle better where I stumbled on first reading - ok? Here I go: There are, essentially, two strategies: One is to explain from the goal backwards: "We want to reach Z; to get this, we must do Y, which we can get by X." The other one from the beginning: "We do X; this gives us Y, which yields Z because of ...". I think a reader must understand always what the current "direction of explanation" is. Here, the explanation is "backwards": "The attacker wants to get valid padding; they do this by setting with the last byte etc." I did not understand this on first reading - I thought "playing" meant "directly playing" and wondered how they could do this. Only two sentences furtheron, I learned that "in order to ... the attacker modifes the ... ciphertext" - aha! The "indirectly" I inserted above hopefully leads the reader along that path right away.
...
%**HMM** Hu? The attacker has chosen R as arbitrary bytes anyway. So why would they XOR 1...256 with r_b? Rather, they will simply take 256 blocks R_0= r_1 r_2 ... 0, R=1r_1 r_2 ... 1, etc. up to R_255 r_1 r_2 ... 255 and try R_k || C_i against the oracle. No need for that XOR here.
...
%**HMM** Isn't the following more correct?
try every possible byte \emph{and} happen to have picked a block that
whose second-to-last byte, when XORed with $R_{b-1}$, happens to be \verb~02~.

In order to successfully decrypt the message, we still need to figure
out which one of those options is the actual value of the padding. 
%**HMM** I think the following would help some readers - we (the readers) are following your text with high concentration, but sometimes we lose track: I thought - "why not just look at P_i? there you can see that last ... oh well, the oracle does not give us P_i! Therefore we need the following roundabout way"
If we had access to a more powerful oracle that would give us the results of
our decryption attempts, we could simply read off that value as the last byte
$p_b$ of the decrypted plaintext. However, our oracle is much more secretive.
Therefore, we need a more roundabout way to elicit this information.
To do that, we try to discover the length of the padding by modifying
...
the block size is 8 bytes. So, $P_i$ is currently:
%**HMM** "we" problem - "we"  the attacker do not know this, only "we" the reader.

\begin{equation}
p_0 p_1 p_2 p_3 p_4 p_5 \mathtt{03} \mathtt{03} \mathtt{03}
\end{equation}

where $p_0 \ldots p_5$ are some bytes of the plaintext. Their actual value
%**HMM**Formatting issue: "where ..." continues the sentence containing that starts with the p_0... formula. Therefore, the paragraph should not be indented. Moreover, "where" should start with a lowercase letter.
...
modify the byte after that (say, we turn that first \verb~03~ into a \verb~02~ via modification of $R$),
$P_i$ looks like this:
...
case scenario that that's only a single byte.) Why? Since we know that the
...
%**HMM** Couldn't we call it $P_i[b]$? No, because $P$ was used above as "some plaintext" (in the decryption) attempts, whereas $D(C_i)$ is /the/ (correct) plaintext. But from where does this complicated name come? Oh, it means "decrypt of C_i" - yes this was introduced a few chapters back, but I and many more readers have long forgotten about this and are now somewhat astonished about that complicated name for a simple byte. Ok, now that I have rambled along ... Could you explain this hard-to-read (and pronounce - therefore hard to remember) notation?
\verb|01|:
...
%**HMM** Again, the "we" continues a sentence (that started at "Since"), it must be lowercase, and the paragraph not indented.
...
%**HMM** Which value? 02? D(C_i) etc.etc.?? I think they actually use $r_b \xor 01 xor 02$. If so, please spell this out here (otherwise, I did not understand the value - so it needs more explanation ...).
possible values for the second-to-last byte.
...
decrypt (and remove padding) emph{before} authenticating the message; so
%**HMM** Why past tense?
the information about the padding being valid leaks despite authentication.
...
However, that turns out to only be a partial solution for systems that decrypt
...
%**HMM** Is this what is called a "timing attack"? In that case, could you mention this term here?
...
%**HMM** Too sleep a little better, could you reveal a little what one does do against these attacks? Even a sentence like "There are now implementations that will answer a decryption attempt in a time that is independent of the fact whether padding is correct or not, which make this sort of attacks useless."; and of course, a pointer to that later citation that says essentially "if you don't authenticate as the first step, you are doing it wrong" would be nice here!
...
%**HMM** What is "synchronous" here? Up to now, all algorithms we saw were, naively, "synchronous" (and not asynchronous, because there was never more than one "thread"). So "synchronous" seems to mean something different in this context. Could you explain that? And knowing the name for a "not synchronous" stream cipher would be helpful (is it "asynchronous", as one would expect?)
