...
git commit with hash \texttt{23d4f9b}, also known as \texttt{v0.1.1-85-g23d4f9b}.
...
Another way to think of XOR is as something called a \enquote{programmable
inverter}: one input bit decides whether to invert the other input
bit, or to just pass it through unchanged. \enquote{Inverting} bits is
colloquially called \enquote{flipping} bits, a term we'll use often throughout
the book.
...
block cipher: it's called the block cipher's \emph{block size}. The set of
...
because it only had to be unpredictable. That's not true: \enquote{secret} is
just a different requirement from \enquote{not secret}, not necessarily a
\emph{stronger} one. It is not generally okay to use secret information
...
case here. There \emph{are} plenty of systems where it is okay to use a
...
of the plaintext: eventually that byte will be \verb~01~, which is always
valid padding. In order to modify the last byte of a plaintext block,
the attacker modifies the last byte of the \emph{previous} ciphertext
block. This works exactly like it did with \hyperref[CBC-bit-flipping-attacks]{CBC bit flipping attacks}.
...
guaranteed a message with a valid \verb~01~ padding, they just have to try
every possible byte. For an attacker to end up with a message with a
valid \verb~02 02~ padding, they have to try every possible byte \emph{and}
happen to have picked a combination of $C$ and $R$ that causes the
plaintext to have a \verb~02~ in that second-to-last position. (To
rephrase: the second-to-last byte of the decryption of the ciphertext
block, XORed with the second-to-last byte of $R$, is \verb~02~.)
...
padding. Let's say that padding is \verb~03 03 03~. Normally, the attacker
...
modify the byte after that (for example, we turn that first \verb~03~ into
\verb~02~ by modifying $R$) $P_i$ looks like this:
...
\emph{timing attack}, which is a special case of a \emph{side-channel attack}:
attacks on the practical implementation of a cryptosystem rather than
its \enquote{perfect} abstract representation. We will talk about these kinds
...
There are also \emph{asynchronous} or \emph{self-synchronizing} stream ciphers,
...
someone says \enquote{stream cipher}, it's safe to assume they mean the
synchronous kind.
...
%**HMM** "This" refers to the "nearest mathching referent" in the previous sentence. In this case, this would be 3DES! I'll that a "referent trouble" in future comments ... "The algorithm" is quite general - but we are in the RC4 section, so it is clear which algorithm is meant. Alternative: "The RC4 algorithm ..."
...
%**HMM** Referent trouble ("this" would reference Chacha or ChaCha8,... here)
Salsa20 is among the state of the art of modern stream
%**HMM** I find "As of time of writing" queer - however, I must accept that is used quite often. I would prefer the more standard (I think) "At the time of writing".
...
%**HMM** Please, no "mathematicians' trivials" ...
...
They can communicate that across the network: it's okay if Eve listens to this communication.
...
Alice as well as Bob then send both of their mixed colors over the network. Eve sees
...
%**HMM** No "computation" here at this point of time - it's just mixing colors.
...
%**HMM** "they" is too risky here - it can reference "Alice and Bob".
...
%**HMM** "check out" is here, IMHO, meant in a colloquial sense that can be confusing.
...
%**HMM** There is no "ciphertext" at the end of a hash function, only a "result" or a "digest" (maybe on some level, "ciphertext" is anything that comes out of something cryptographic that can't be read easily - but this is a stretch of the wording at least for us readers; and anyway, you consistently use the word "digest" in this section except for this one place.
