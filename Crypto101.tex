...
\label{sec-2-4-2}
%**HMM** "Abstract" is somewhat of a misnomer here. The color example is no more abstract than a "bit-sequence" DH. The operations are just as concrete. Suggestions: "An explanation Diffie-Hellman with color mixing", "Toy Diffie-Hellman with colors", "Diffie-Hellman explained with color mixing", "Diffie-Hellman explained with colors", ...mhm... "Abstract Diffie-Hellman" is still ok, but not the explanations further down that "abstract" operations are now made "concrete" - I'll leave a note there ...
...
Alternatively, and typically, this base color is a fixed part of the protocol; Alice and
Bob then don't need to communicate it. After this step, Alice, Bob and Eve
...
Please note that Eve cannot just mix the two mixes she has got:
white plus red plus white plus yellow---i.e., Eve's "mix of mixes"---is 
different from white plus red plus yellow---i.e., Alice's and Bob's shared
secret. The former is somewhat lighter, because it has the double amount
of white color in it.
...
This section describes another practical implementation of the
Diffie-Hellman algorithm, this time based on the elliptic curve discrete
...
%**HMM** larger??? i.e., they are slower??? No: smaller (as they are faster)
...
%**HMM**Impressive formula - but it doesn't tell me much. I reduced it a little: = O(e^((64/9)*(log n)^(1/3)*(log log n)^(2/3)) <= O(e^((64/9)*(log n)^(1/3)*log log n) = O((e^log log n)^(64/9)*(log n)^(1/3)) = O((log n)^(64/9)*(log n)^(1/3)) <= O((log n)^(8*log n) .... and this is strictly smaller than O(sqrt(n)), if I made no errors. No need to add something like that, but an explaining sentence like the following would help:
This scary-looking expression is essentially something that is smaller than a logarithm raised to a logarithmic power.
...
For growing n, this term will get larger than the logarithmic term above at some point (which depends on the constant factors hidden in the O notation).
...
%**HMM** What are RSA and EC? Please refer to the Public-key chapter for RSA, and expand EC (I can guess both, but I don't want to :-) )
...
%**HMM** What is ECC? Please expand (I can guess, but I don't want to :-) )
\hline
56 & 512 & 112\\
80 & 1024 & 160\\
112 & 2048 & 224\\
