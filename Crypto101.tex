...
%**HMM** Usually, you gice specific links - here it's only "in a previous chapter"; and, alas, it has never been noted. There's only a general remark in the "Remaining problems" section after "one-time pad", but without any concrete formula.
%**HMM** Moreover, as people do no need to encrypt communication with themselves, n^2 is wrong, isn't it? It should be $n(n-1)$; or "roughly $n^2$" or "almost $n^2$", if wou want to keep this simple formula.
between $n$ people, we need $n^2$ key exchanges. While the key to be
exchanged is a lot smaller now than with one-time pads, the fundamental problem of the
