...
medical, payroll or criminal records. The server protects the information 
by encrypting it with a strong block cipher in :term:`CBC mode` and a fixed key. For now, we assume the
server is secure, and no way for a key leak.
