...
This scheme doesn't have an ubiquitous name. The one we'll use in this
book is "Prefix-MAC", because it is a MAC algorithm that works by
using the secret key as a prefix.
...
This construction is actually secure in many modern, SHA-3 era hash
functions such as Keccak (the algorithm behind SHA-3) and BLAKE(2); in
fact, the specifications for these functions recommend it as a secure
and fast MAC.
