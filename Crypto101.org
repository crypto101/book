...
1. You can apply XOR in any order: $a \xor b = b \xor a$, no matter
   what values $a$ and $b$ are (also known as /commutative/).
...
AES requires seperate keys for each round in the next steps. The key
schedule is the process which AES uses to derive 128-bit keys for
each round from one master key.

First, the key is seperated into 4 byte columns. The key is rotated
and then each byte is run through an S-box (subsitution box) that
