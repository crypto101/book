...
First, we'll exploit the fact that repeating input blocks result in
repeating ciphertext blocks, by visually inspecting an encrypted
...
ciphertext. In this section, we'll study an /active/ attack, where
the attacker actively communicates with their target, can enable an
attacker to decrypt ciphertexts encrypted using ECB mode.
...
data, followed by a secret prefix $S$, in ECB mode. Or, in symbols:
...
The goal of the oracle is for those to remain secret, but we'll see
how an attacker can recover $S$ by inspecting the ciphertext $C$ for
many carefully chosen values of the prefix $A$.

TODO: This would undoubtedly benefit from a bunch of illustrations.
...
of the first encrypted block: $E_k(A \| s_0)$.
...
know the guess is correct because the resulting block will match the
block they remembered earlier.
...
encrypt data all the time. We've seen that when using \gls{ECB mode},
an attacker can both analyze ciphertexts to recognize repeating
patterns, and even decrypt messages when given access to an
...
byte of the block $C_i$. He can also trick them into decrypting the
...
secret values (the /keys/) from one secret value. Many key derivation
functions additionally take an (often optional) /salt/, so that
different invocations of the key derivation function won't always
return the same output keys for the same input secret. As with other
cryptosystems, salts are fundamentally different from the secret
input: salt do not have to be secret, and can be re-used.
...
password will take. As we've already established in [[password%20storage][a previous section
on password storage]], that means it is necessary that the key
derivation function is hard to compute, requiring a non-trivial amount
of computing resources, such as CPU cycles or memory. If the key
derivation function were easy to compute, an attacker could simply
enumerate all possible values of the shared secret, since there are
few possibilities, and then compute the key derivation function for
all of them. As we've seen in that previous section on password
storage, this is how most modern attacks on password stores work.
Using an appropriate key derivation function, such as scrypt, would
prevent these attacks. In this chapter, we'll see scrypt, as well as
other key derivation functions in this category.
