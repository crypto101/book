...
one of the most popular native stream ciphers, has had serious known
issues for years. By comparison, some of the constructions using block
...
quoted bencmarks don't mention RC4 but MARC4, which stands for
...
Instead of reusing existing complete cryptosystems, it implements its
...
As the nonce's encryption is totally unpredictable, this will, in the
eyes of an attacker, randomly flip the bits of the result of the
one-time MAC. Therefore, no algebra whatsoever can exploit the
re-use of $k_2$.

Keep in mind that while Carter-Wegman MACs take two distinct keys $k_1$
and $k_2$, and that Carter-Wegman MACs are related to one-time MACs
which also take two distinct keys $a$ and $b$, these keys are unrelated. The
...
two keys $a$ and $b$, that $k_2$ would have to get split up into those two keys.
...
For simplicity's sake, our demonstration 12-hour clock only shows hours, not minutes
or seconds. And also unlike real clocks, the hour hand always shows an exact
...
equation and use an $\equiv$ sign instead of an equals sign ($=$):
...
modulus/ if dividing them by the modulus leaves the same remainder.
You can check this out with the examples above: $10 + 4 = 14$ leaves
a remainder of 2 when divided by 12, and so does 2. For negative
numbers, the remainder is always assumed to be the positive one,
therefore $2 - 5 = -3$ leaves a remainder of $+9$, as the next lower
multiple of 12 is $-12$, which is 9 below $-3$.
...
As a side note, it may be worth mentioning that 1 is not supposed to be
a prime number, even though it is only divisible by 1 and itself (1 again).
The reason is that by this convention, the factorization of any number
is unique. Otherwise, 4 could be factored not only as 
$2 \cdot 2$, but also as $2 \cdot 2 \cdot 1$, $2 \cdot 2 \cdot 1 \cdot 1$, 
etc. The uniqueness of factorization helps in some important proofs in
number theory.

Also, as a fun fact, 0 is /not/ a prime number, as it is divisible
by many numbers---in fact, by all numbers except 0.
...
2 when divided by 7; so does 2, of course. 
...
amount of numbers that are coprime to its argument. As an example,
the totient of 10 is 4, as 1, 3, 7, and 9 do not have common prime factors
with 10.

Here is how the totient is used to find $a^{-1}$.
Multiplying both sides of Euler's theorem by $a^{-1}$, we
get:
...
How this is done efficiently is explained in the next section.
