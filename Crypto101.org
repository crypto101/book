...
medical, payroll or even criminal records. In order to protect the
secret user information, the server that handles it encrypts it using
a strong block cipher in CBC mode with a fixed key. For now, we'll
assume that that server is secure, and there's no way to get it to
leak the key.
...
Mallory wants to figure out what Alice's record says. Right now, all
Mallory can see is Alice's ciphertext. If we assume there's only one
ciphertext block for simplicity's sake, Alice's ciphertext consists of
an IV and one ciphertext block.
...
$G$ as to what Alice's data could be, like ~NO CRIMINAL RECORD~. She
asks the server to encrypt:
...
C_M & = E(k, IV_M \xor P_M) \\
    & = E(k, IV_M \xor (IV_M \xor IV_A \xor \mathtt{NO CRIMINAL RECORD})) \\
    & = E(k, IV_A \xor \mathtt{NO CRIMINAL RECORD})
\end{align*}
#+END_LATEX

That ciphertext, C_M, is exactly the ciphertext block Alice would have
had if her plaintext block was ~NO CRIMINAL RECORD~. So, if the blocks
match, Mallory knows Alice has no criminal record; if they do, she
knows she does, and can start using that against her...

Lessons learned: don't roll your own cryptosystems. Don't let IVs be
predictable. Authenticate your messages.
