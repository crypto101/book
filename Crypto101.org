...
used ofr many years to come, unless there are some large cryptanalytic
...
This scheme is called "ECB mode". There is an obvious flaw with it:
...
At first, this might not seem like a particularly serious problem. If
the properties of the block cipher hold, an attacker still wouldn't
be able to determine what the plaintext was, just from this stream,
only that the plaintext repeated.
...
it's just less visual.]. We then visually inspect the different
outputs.
...
#+BEGIN_LATEX
  \begin{figure}[p]
    ...
    \caption{Plaintext image with ciphertext images under idealized
      ...
      block size is realistic.}
  \end{figure}
#+END_LATEX

Because identical blocks of pixels in the input map to identical
blocks of pixels in the output, the global structure of the image is
mostly preserved.
...
Using ECB mode with a plaintext consisting of an attacker-controlled
part followed by some secret data, allows the attacker to decrypt a
block's worth of that secret data.
...
ciphertext $C$ that encrypts some attacker-provided input $A$
followed by a secret suffix $S$ in ECB mode under key $k$:
...
For a typical block size of 16 bytes, brute forcing would mean trying
$256^{16}$ combinations. That's a huge number, consisting of 39
digits, a number large enough that trying all combinations is
considered impossible. This attack allows them to do it in at most
$256 \cdot 16 = 4096$ tries, a far more manageable number.

**** Conclusion

In the real world, block ciphers are typically used to encrypt data.
However, none of these issues apply. Even when we use idealized block
ciphers with unrealistic properties, such as block sizes of more than
a thousand bits, or even if we use block ciphers of /any/ block size,
we can discover many things about the data.

Real world block ciphers obviously have many more limitations. This
isn't even taking into account any weakness in the block cipher. It's
not AES (or our test block ciphers) that cause this problem, it's our
construction. Clearly, we need something better.
...
construction. Initialization vectors come back in many other
algorithms.
...
Decryption, is, obviously, the inverse construction, with block
ciphers in decryption mode instead of encryption mode:

[[./Illustrations/CBC/Decryption.pdf]]

While CBC mode itself is not inherently insecure (unlike ECB mode),
its particular use in TLS 1.0 was. This eventually led to the BEAST
attack, which we'll cover in more detail in the section on SSL/TLS.
The short version is that instead of choosing truly random IVs, the
previous ciphertext block was used. That sounds reasonable at face
value: it's as if you had one very long CBC transmission chain.
...
essentially leading to a similar problem to what we saw with [[ECB mode]].
