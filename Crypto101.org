...
i.e., the hash digest for some lengthened message,
without actually knowing the value of $M_1$. 
Why is this a potential problem? Here is a somewhat silly example:
Assume that we can add the text $M_2$ = \verb*|", we thought. But actually, it
would be better to transfer $500,000 to account XYZ."| to a message $M_1$,
then re-authenticate it with $H(M_1 \| M_2)$ and send it on to the
receiver, where it would pass the authentication check! A practical
and more scary attack will be shown in the chapter on MACs
# **HMM** ... with the key-value-pair code ....

How does this sort of attacks work? Since you know $H(M_1)$,
you know the state of the hash function after it has hashed $M_1$. You
can use that to "reconstruct" the hash function, i.e., set the state
of an implementation exactly to $H(M_1)$ (setting the hash function's 
internal state to a known state you got from somewhere else 
is called /fixation/). After that, you can just let it continue the
computation for additional input bytes.
...
For the attacker to be able to compute with such a hash function
the value $H(M_1 \| M_2)$ when knowing
...
# **HMM** As always, such a (maybe partly redundant) information is helpful for us readers to check that we understand what's going on.
# **HMM** Is a "/meet/ in the middle attack" the same as a "/man/ in the middle attack"? That would deserve clarification.
