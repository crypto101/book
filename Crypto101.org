...
Storing the password directly has several issues. 
If the password database were
# **HMM** For us (still newbies in crypto), that timing attack is everything but obvious. I think it's better to leave out this problem here altogether - see also issu #148.
...
# *HMM** or XORing? Of course, that's a bad idea, as the length of the salt is then truncated to the length of the password. Pointing out this problem might be worth a footnote - I assure you, some people think this to be a good idea, because "XORing scrambles so much more than contatenation, doesn't it?".
...
salt with the password, hash it, and compare the result against the stored
hash.

If you pick a random value as salt that's large enough (say, $2^{160}$, i.e., 32 bytes),
...
[[key derivation function][key derivation function]]. This is a quite
different thing from a naked cryptographic hash function, and
we'll deal with those in a following chapter.
Here, we continue with attacks on hash functions---which can usually
be turned into the attacks on the cryptographic algorithms using them.
