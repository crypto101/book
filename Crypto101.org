...
This scheme is called "ECB mode". Unfortunately, while being very
...
the properties of the block cipher hold, an attacker still wouldn't be
able to determine what the plaintext was just from this stream, only
that some plaintext block was repeated.
...
#+BEGIN_LATEX
  \begin{figure}[p]
    ...
    \caption{Plaintext image with ciphertext images under idealized
      ...
      block size (5 pixels or 120 bits) is realistic.}
  \end{figure}
#+END_LATEX
