...
half a percent ballpark. (In the next step, the attacker will attempt
to produce valid padding. If by pure chance they happen to already
have valid padding, they can simply skip that step.)
...
The first option (~01~) is much more likely. It only requires one byte
to have a specific value; all the other ones require multiple bytes to
be some specific value.

In order to successfully decrypt the message, we still need to figure
out which one of those options it was. To do that, we start modifying
bytes starting at the left-hand side of the block, until padding
breaks again. As soon as padding breaks, you know that byte was part
of the valid padding, which tells you how many bytes there are, and,
since we're using PKCS#5 padding, what their value is.
