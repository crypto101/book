...
In late 1997, the NIST announced their plans to replace the old
Data Encryption by the new Advanced Encryption Standard, or \gls{AES}
for short. Unlike with DES, the standardization and design of AES was
open and featured an, amongst cryptographers, well-received competition
that also included conferences where algorithms were presented, broken,
and voted upon.

In the end, an algorithm called Rijndael, developed by 2 belgian
cryptographers, was selected to become AES. Rijndael defined a
...
\cite{fips:aes}. Today, AES is the most commonly used block cipher.
...
Unlike several other block ciphers, including the predecessor DES,
AES is not using a \gls{Feistel network}, but is based on a
Substitution-Permutation network (SPN), where each building block
has only one particular role. This, however, also means that each
function has to be invertible, i.e. bijective.

TODO: Actually explain how AES works :)

***** SubBytes

SubBytes is the S-box of AES, being $8\times 8$ bits in size. It works
by taking the multiplicative inverse over the Galois Field, and then
applying an affine transformation so that there's no value $x$ with
either $x\oplus S(x)=0$ nor $x\oplus S(x)=\texttt{0xff}$. The affine
transformation is thus removing easy-to-exploit differentials, which
was an issue with the fifth S-box in the DES.
...
After having applied the above SubBytes step to each single of the
$16$ bytes of a given block, AES then shifts the rows in the
$4\times 4$ array by a certain amount:
...
MixColumns takes a column of the state, multiplies it with a fixed
polynomial, and replaces the column with the result.

ShiftRows and MixColumns together are the important pieces to distribute
changes across the entire state as quickly as possible.
...
As the name implies, AddRoundKey xors bytes from the key schedule to the
bytes of the state.
...
\illustration{StreamCipher/Synchronous}[.8][A synchronous stream cipher scheme. Note similarity to the one-time pad scheme.]
#+END_LATEX
...
most demanding randomness tests. Despite all of these, it is /not/
cryptographically secure. To demonstrate this, we'll take a look at
how the algorithm works. Fortunately, it's not very complex.
...
secret value, such as a Diffie-Hellman secret or the RSA decryption
exponent $d$.
...
mathematics works for many other systems. [fn::Specifically, it works
for any abelian group.]

**** Deriving the ladder

In this subsection, we'll see how you can discover this algorithm.
This requires a decent bit of math; if you're not really interested in
how you get to a Montgomery ladder and just want to see how it works,
