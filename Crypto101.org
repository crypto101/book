...
can perform a so-called man-in-the-middle chosen-ciphertext attack to recover
# **HMM** That's the first time in the book that "man-in-the-middle ... attack" occurs: Being a very common term /and/ an important concept, I think it deserves an explanation. However, this might not be the right place, because here we are in a section that many readers might skip ("just a detail on CBC attacks"). A better place would be in the DH chapter with Mallory being that "active attacker".
the key.
...
# **HMM** Here is that place where an explanation of "man-in-the-middle" attack would nicely fit in:
Such attacks are called "man-in-the-middle attacks," for obvious reasons.
Given that the network infrastructure which we use to send messages
is run by many different operators, it is virtually impossible to
exclude that such a "man in the middle" is present in some message exchange.
...
# **HMM** ...e.g. in the man in the middle attack we saw in the DH chapter.
...
private-key (symmetric) and public-key (asymmetric) forms. Symmetric authentication
# **HMM** "asymmetric" is introduced too casually here (its only previous mention was as part of the name OAEP). Also, that "symmetric" is the opposite of "asymmetric" deserves at least one sentence (of course, it's obvious - but a book like this is about explaining also things that are obvious - at least, this gives us readers sometimes the feeling that we are still abreast with what's going on ;) )
schemes are typically called message authentication codes. Public-key
authentication is typically called a signature.

But before we deal with authentication, we first introduce a new cryptographic primitive: hash
