...
also pick a key length $L$ and an additional number $N$. While the original
...
multiplicative order[fn::Given integer $a$ and positive integer $b$ with
gcd$(a, b) = 1$, the multiplicative order of $a mod b$ is the smallest
positive integer $k$ such that $a^k = 1 mod b$] $mod p$ is $q$. The easy
way to do this is to set $g = 2^(p-1)/q mod p$. We can try another number
...
$y$ where $y = g^x mod p$. This delivers a public key $(p, q, g, y)$, and
private key $x$.
