...
Although this looks like a lot of computation, this polynomial can be 
easily evaluated by Horner's method, i.e., by iteratively computing the
sum from the right. When one evaluates the modulo operator in each step,
the numbers will remain conveniently small.
...
they are being mixed irreversibly only once. This demonstrates that the MAC is
...
starting by subtracting the two equations (for the modular
inverse, see the appendix on modular arithmetic):
...
securely, over the Internet or any other insecure medium. It's a hybrid
...
peers, and public key algorithms can be used to negotiate shared secrets
