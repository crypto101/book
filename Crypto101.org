...
simplest: concatenate[fn:: Here we use \| as the operator for 
concatenation; this symbol is often seen as the "logical or" operator. 
Other common symbols for concatenation include + (for some programming 
languages) and a center dot (for formal languages).] 
