...
impossibly large number of key exchanges hasn't been solved yet. We
will tackle that problem in the next section, where we'll look at key
exchange protocols: protocols that allow us to agree on a secret key
...
On the other hand, someone who does not know $d$ cannot decrypt the 
message. But, and that's the whole point, it is /virtually impossible
to compute $d$ from $N$ and $e$ alone/. That's the reason why RSA was
such a breakthrough in encryption.
...
# **HMM** This footnote is not a sentence ("For a variety of reasons, which ... are secondary") and is horrible to parse (two nested dependent phrases, a present participle, "tenet" being a head-scratching word for many of us ... I don't understand what you want to tell us, and have the gut feeling that it's not worth the whole footnote, at least at this place?? Or, simply said, this footnote is too complex.
...
encryption phase didn't actually do anything: $M^1 = M$, of course,
so the "encrypted" message was actually the plaintext. While this issue has
...
takes a random block $R$ that's $k$ bits long, where $k$ is a constant specified by
# **HMM** The relative clause "a constant ..." can also be tied to $R$, which would be wrong.
