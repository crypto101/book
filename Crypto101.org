...
attacker has successfully forged a tag for a new message without
knowing the key, and so, by our definition, the MAC is insecure.
...
forge are for extensions of messages that we sent and signed;
...
can effectively control the parsed data entirely with a length
extension attack.
