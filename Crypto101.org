...
secret values from one secret value. It can be useful, for example,
when a cryptographic protocol starts with a single secret value, such
as a shared password, or a secret derived using Diffie-Hellman key
exchange, but requires multiple secret values to continue, such as
