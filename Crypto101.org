...
git commit with hash call_gitcommit() and description call_gitdesc().
...
means approximately 2500 cycles.
...
This scheme doesn't have a terribly common name; perhaps the most
common one (which we'll use in this book) is Prefix-MAC; because it is
a MAC algorithm that works by using the secret key as a prefix.

The cryptographically secure hash function $H$ guarantees two things
that are important to us here:

 - The tag $t$ will be easy to compute; the hash function $H$ itself
   is typically very fast, and in many cases we can precompute the
   common key part, so we only have to hash the message itself,
 - Given any number of tags $t_i = H(k \| m_i)$, there is no way for
   an attacker to "invert" the hash function to recover $k$, or even
   "rewind" the hash function to recover $H(k)$; being able to do
   either of them would obviously allow them to forge arbitrary
   messages.

One small caveat: the latter point assumes that your secret key $k$
has enough entropy. Otherwise, we have the same issue that we had for
password storage using hash functions: an attacker could just try
every single $k$ until one of them matches. Once they've done that,
they've almost certainly found the correct $k$. That's not really a
failure of the MAC though: if your secret key contains so little
entropy that it's feasible for an attacker to try all of them, you've
already lost, no matter which MAC algorithm you pick.

**** Breaking the Prefix-MAC
...
problem: now the attacker can forge messages.
...
attacker has successfully forged a tag for a new message, and the MAC
is insecure.
...
This attack might sound a little academic, and far from practical. We
may have proven that the MAC is insecure by our definition, but the
only signatures the attacker can successfully forge are for
modifications of messages that we sent and signed; specifically, the
message we sent, followed by some binary junk, followed by something
the attacker chooses. It turns out that for many systems, this is
...
Sure, that code has many issues. For example, it doesn't handle
escaping correctly. But even if it did; that wouldn't really fix this
problem. Hopefully, it illustrates that there is in fact a pretty good
chance that an attacker can produce messages with valid tags that say
something entirely different from what you intended.

Under many modern, SHA-3 era hash functions such as Keccak (the
algorithm behind SHA-3) and BLAKE2, this construction is in fact
secure.
...
TODO: link http://cseweb.ucsd.edu/~mihir/papers/hmac-new.html
