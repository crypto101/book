...
secrets. Bob, on the other hand, just has some secrets shared with Mallory,
but he has no way to prove that to Alice. As far as Alice can tell, Bob just
chose a few random numbers. There's no way to link any key that Bob has with
any key that Alice has.
