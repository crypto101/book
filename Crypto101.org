...
\]
#+END_LATEX
# **HMM** According to standard notation, and your appendix on modular arithmetic, this is not a well-formed formula: One writes (mod p) only at the /end/ of a formula, where it guards the relational operator; and, in that case, one writes \equiv instead of =. Earlier equations (e.g. for one-time MACs) already had = instead of \equiv, but there one could regard it as a "minor hiccup". Here, you deviate from the notation you describe in your appendix. Maybe standard \bmod instead would just be fine ...
...
can add 5, and end up with 7, as you can see in the next figure.
...
In the general case, we call two numbers are equivalent modulo some 
...
\pmod 7$. Check it out: $5 \cdot 6 = 30$, which leaves a remainder of
...
thi is still generally less interesting than using the extended
...
\]
#+END_LATEX
# **HMM** Again: \equiv, as "prescribed"; or go with = even when there is (mod p) at the end?
