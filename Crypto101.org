...
other. Each participant has a long-term DSA authentication key, which
we'll call ($pub_A,pr_A)$ and $(pub_B,pr_b)$ respectively.
...
sends $AES_r(g^x)$ and $HASH(g^x)$ to the peer.
...
As we saw before, Diffie-Hellman does not /authenticate/ the peer. Bob
can compute a secret, but doesn't know he's talking to Alice. As with
TLS and other systems using Diffie-Hellman, this problem is solved by
...
He chooses an identification number $keyid_B$ for his current
...
Bob computes $M_B = MAC_{m_1}(g^x, g^y, pub_B, keyid_B)$ and $X_B =
(pub_B, keyid_B, sig_B(M_B))$. He sends Alice $r, AES_c(X_B),
MAC_{m_2}(AES_c(X_B))$.
...
key. She then computes $HASH(g^x)$, to check that it is indeed the
same value as what Bob had previously sent. She then computes $s =
(g^x)^y$ and derives the same keys: $c, c\prime, m_1, m_1\prime, m_2,
m_2\prime$. Using $m_2$, she can verify $MAC_{m_2}(AES_c(X_B))$. Once
that message is verified, she can safely decrypt it using her computed
$c$.

She can then also compute $M_B = MAC_{m_1}(g^x, g^y, pub_B, keyid_B)$,
and verifies that it is the same as Bob sent. By verifying
$sig_B(M_B)$ with Bob's public key, she has now unambiguously tied the
current interaction to Bob's long-term authentication key.
...
her. She chooses an identification number $keyid_A$ for her current DH
keypair $(y, g^y)$, computes $M_A = MAC_{m_1\prime}(g^y, g^x, pub_A,
keyid_A)$ and $X_A = pub_A, keyid_A, sig_A(M_A)$. Finally, she sends Bob
...
Once he decrypts $AES_{c\prime}(X_A)$, he has access to Alice's
long-term public key information, and he can compute $M_A =
MAC_{m_1\prime}(g^y, g^x, pub_A, keyid_A)$ to compare it with the
version Alice sent. Finally, he verifies $sig_A(M_A)$ with Alice's
public key.
