...
<<keystream jump>>
Salsa20 has a few particularly interesting properties. Firstly, It's 
...
done by ensuring that no key material is ever used to make a decision
in the cipher, and that every round is made up of a fixed-number of 
constant-time operations. The result is that every block is produced
with exactly the same number of operations, regardless of what the 
key is. 
