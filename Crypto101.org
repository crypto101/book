...
Both stream ciphers are based on ARX ciphers (Add-Rotate-XOR). A good characteristic of ARX is the fact that it prevents timing attacks, since it runs in constant time. On top of that, it performs well on CPUs, because it doesn't need hardware support to function well.
...
First we perform a modular addition (⊞) of y and z, then we XOR ($\xor$) the result with x and finally we rotate left (<<<) by n bits.
