...
add 5, and end up with 7, as you can see in figure
:raw-latex:`\ref{fig:Clock2Plus5}` on page
:raw-latex:`\pageref{fig:Clock2Plus5}`.
...
:math:`a \cdot b
\equiv c \pmod m`, then :math:`\frac{c}{b} \equiv a \pmod m`.

For example, :math:`5 \cdot 6 \equiv 2 \pmod 7`; so:
:math:`\frac{2}{6} \equiv 5
\pmod 7`. This is because :math:`5 \cdot 6 = 30`, which leaves a
...
just like the inverse of a number in regular arithmetic: :math:`x \cdot
\frac{1}{x} = 1`.
...
#. There's exactly one identity element :math:`i`, for which
   :math:`a \star i = i
     \star a = a`. For integer addition, that's zero:
   :math:`a + 0 = 0 + a =
     a` for all a.
